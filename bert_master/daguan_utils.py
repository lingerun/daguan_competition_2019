...
train_aug_path = os.path.join(base_dir,'datagrand/train_augmentation.txt')
    ...
    with open(path,'r') as f:
        for i, line in enumerate(f.readlines()):
            ...
            if i==1757:
                ...
                print(lens)
        max.sort()
        print('The count length: all--{} /n <50: {}, <100: {}, <200: {}, <300: {}, 300+: {}'.format(all, count_50,count_100,count_200,count_300,other_300))
        print(max)
        print(index)

def trans_corpus_bert(path):
    item = path.split('.txt')
    item[-1] = '_bert.txt'
    out_path = ''.join(item)
    print(out_path)
    ...
    with open(path,'r') as f:
        for line in f.readlines():
            line_process = line.replace('_', ' ')
            wf.write(line_process)

    wf.close()


def get_label_words(path):
    words_a = []
    words_b = []
    words_c = []
    with open(path,'r') as f:
    ...
    print(words_b)
    print(words_c)
    print('words_a: {}, words_b: {}, words_c: {}'.format(len(words_a),len(words_b),len(words_c)))
    return list(words_a),list(words_b),list(words_c)

def data_augmentation(path):
    item = path.split('.txt')
    item[-1] = '_augmentation.txt'
    out_path = ''.join(item)
    print(out_path)

    aug_num = 3  #每个语句中每个类别增强的数目

    #获取每类对应的词语集（带/c /b /a 三类）
    words_a,words_b,words_c = get_label_words(path)
    ...
    with open(path,'r') as f:
        for line in f.readlines():
            ...
            #写入原始语句
            aug_lines.append('  '.join(items))
            for i, item in enumerate(items):
                label = item[-1]


                if label == 'a':
                    #随机从words_a中取k个词语替换演示词语
                    rand_index = random.sample(range(len(words_a)), k=aug_num)
                    for index in rand_index:
                        items[i] = words_a[index]
                        aug_lines.append('  '.join(items))

                elif label == 'b':
                    rand_index = random.sample(range(len(words_b)), k=aug_num)
                    for index in rand_index:
                        items[i] = words_b[index]
                        aug_lines.append('  '.join(items))

                elif label == 'c':
                    rand_index = random.sample(range(len(words_c)), k=aug_num)
                    for index in rand_index:
                        items[i] = words_c[index]
                        aug_lines.append('  '.join(items))

                else:
                    continue

        print('orig: {}, aug: {}'.format(len(f.readlines()),len(aug_lines)))


    with open(out_path,'w') as wf:
        for text in aug_lines:
            wf.write(text + '\n')
...
if __name__ == '__main__':
    ...
    # #获取句子长度计数
    # get_length_corpus(test_path)

    # #corpus处理，_换成空格，便于bert预训练
    # trans_corpus_bert(corpus_path)

    # #获取训练集每一类别所有的词，用于数据增强
    # get_label_words(train_path)

    # #数据增强,先获取相同类别的词，在进行对应位置的替换，再通过turn_aug_BIO转化标记
